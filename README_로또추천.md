...
## 3) 당첨번호 **완전 자동 갱신** (수동 엑셀 없음)

동행복권 API로 당첨번호를 자동 수집해 `data.json`과 `docs/`를 갱신할 수 있습니다.
...
- 엑셀 파일 없이 **1회차 ~ 최신 회차**까지 API로 받아와 `static/data.json`, `docs/data.json`, `docs/index.html`을 만듭니다.
- 끝난 뒤 `docs/` 를 커밋·푸시하면 GitHub Pages에 반영됩니다.

### GitHub에서 매주 자동 실행

- 저장소에 **`.github/workflows/update-lotto.yml`** 이 있으면, **매주 월요일 09:00(한국 시간)** 에 GitHub Actions가 자동으로:
  1. `update_lotto_auto.py` 실행 (API로 최신 당첨번호 수집)
  2. `docs/` 와 `static/data.json` 이 바뀌었으면 커밋 후 푸시
- 수동으로 엑셀 받거나 `build_static_data.py` 를 돌릴 필요 없습니다.
- 수동 실행: GitHub 저장소 **Actions** 탭 → **Update Lotto Data** → **Run workflow**.
